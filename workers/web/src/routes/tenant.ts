...
import { mountTenantAdminSpa } from "./admin/index.js";
...
export function mountTenant(app: Hono<{ Bindings: Env; Variables: AppVariables }>): void {
  ...
  // Tenant admin SPA at /admin and /admin/*. The SPA boots, calls /api/me,
  // sees `kind: "tenant"`, and renders tenant admin views (groups, members,
  // pending, team, settings).
  mountTenantAdminSpa(app);
