/**
 * requireSiteAdmin — Hono middleware for app.<apex>. Checks Host classification,
 * verifies bm_site_session cookie, loads the site admin into `c.var.siteAdmin`.
 * Returns 404 on wrong host (don't leak the API surface), 401 when no/invalid
 * session.
 */
...
export const requireSiteAdmin: MiddlewareHandler<{
  ...
  if (classifyHost(host, c.var.config).kind !== "admin") {
